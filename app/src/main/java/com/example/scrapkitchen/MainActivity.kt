...
import com.example.scrapkitchen.databinding.ActivityMainBinding
...
//        seeRecipes(recipeViewModel.getRecipes())
    ...
    fun seeRecipes(recipes: List<RecipeEnt>){
        var catmap = HashMap<String,Int>()
        var metricmap = HashMap<String,Int>()
        var namemap = HashMap<String,Int>()
        recipes.forEach {
            ...
            if (ingredientList.size % 4 != 0){
                ...
                for (i in 0..ingredientList.size - 1 step 4) {
                    var cat = ingredientList[i+2].trim().lowercase()
                    var metric = ingredientList[i+1].trim().lowercase()
                    var name = ingredientList[i+3].trim().lowercase()
